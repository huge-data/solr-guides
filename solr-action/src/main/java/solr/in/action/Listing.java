...
public class Listing implements ExampleDriver.Example {
	...
	static {
		...
		listings.put(
				"4.4",
				"/collection1/select?q=iPod&fq=manu%3ABelkin&sort=price+asc&fl=name%2Cprice%2Cfeatures%2Cscore&df=text&wt=xml&start=0&rows=10");
		listings.put("4.7", "/collection1/browse?q=iPod&wt=xml&echoParams=all");
		listings.put("4.8", "/collection1/select?q=*%3A*&wt=xml&stats=true&stats.field=price");
		listings.put("4.9", "/collection1/browse?q=iPod&wt=xml&debugQuery=true");
		listings.put(
				"4.12",
				"/collection1/select?q=iPod&fq=manu%3ABelkin&sort=price+asc&fl=name%2Cprice%2Cfeatures%2Cscore&df=text&wt=xml&start=0&rows=10");
		...
		listings.put(
				"8.11",
				"/restaurants/select?q=*:*&facet=true&facet.mincount=1&facet.field=state&facet.field=city&facet.query=price:[*%20TO%2010%7D&facet.query=price:[10%20TO%2025%7D&facet.query=price:[25%20TO%2050%7D&facet.query=price:[50%20TO%20*]&fq=state:California");
		listings.put(
				"8.12",
				"/restaurants/select?q=*:*&facet=true&facet.mincount=1&facet.field=state&facet.field=city&facet.query=price:[*%20TO%2010%7D&facet.query=price:[10%20TO%2025%7D&facet.query=price:[25%20TO%2050%7D&facet.query=price:[50%20TO%20*]&fq=state:California&fq=price:[*%20TO%2010%7D");
		listings.put(
				"8.13",
				new String[] {
		...
		listings.put(
				"10.8",
				"/solrpedia/select?q=northatlantic+curent&df=suggest&q.op=AND&spellcheck.dictionary=wordbreak&spellcheck.dictionary=default");
		listings.put("10.10", "/solrpedia/suggest?q=atm");
		listings.put("10.15",
				"/solrpedia_instant/select?q=query_ngram:willia&sort=popularity+desc&rows=1&fl=query&wt=json");
		listings.put(
				"10.16",
		...
		listings.put("14.12", "/langid/select?q=*:*&fl=title,language,languages");
		listings.put(
				"14.14",
		...
		listings.put("14.17", "/multi-langid/select?q=*:*&df=content&fl=title,content,language");
		listings.put("14.18", "/multi-langid/select?q=en,es,fr%7Cabandon&df=content&fl=title,content,language");

		// chapter 15: Performing Queries and Handling Results
		listings.put("15.1",
				"/salestax/select?q=*:*&userSalesTax=0.07&fl=id,basePrice,totalPrice:product(basePrice,%20sum(1,%20$userSalesTax))");
		listings.put("15.5", "/geospatial/select?q=*:*&fl=id,city,distance:geodist(location,37.77493,%20-122.41942)");
		listings.put(
				"15.6",
				"/geospatial/select?q=*:*&fl=id,city,distance:geodist(location,37.77493,%20-122.41942)&sort=geodist(location,37.77493,%20-122.41942)%20asc,%20score%20desc");
		listings.put(
				"15.8",
				new String[] {
		...
		listings.put(
				"16.2",
				"/no-title-boost/select?defType=edismax&q=red%20lobster&qf=restaurant_name%20description&fl=id,restaurant_name,description,[explain]");
		listings.put(
				"16.3",
				"/title-boost/select?defType=edismax&q=red%20lobster&qf=restaurant_name%20description&fl=id,restaurant_name,description");
		listings.put(
				"16.5",
				"/jobs/select/?fl=jobtitle,city,state,salary&q=(jobtitle:%22nurse%20educator%22%5E25%20OR%20jobtitle:(nurse%20educator)%5E10)%20AND%20((city:%22Boston%22%20AND%20state:%22MA%22)%5E15%20OR%20state:%22MA%22)%20AND%20_val_:%22map(salary,%2040000,60000,10,0)%22");
		listings.put(
				"16.6",
				"/jobs/select/?df=classification&q=((%22healthcare.nursing.oncology%22%5E40%20OR%20%22healthcare.nursing%22%5E20%20OR%20%22healthcare%22%5E10)%20OR%20(%22healthcare.nursing.transplant%22%5E20%20OR%20%22healthcare.nursing%22%5E10%20OR%20%22healthcare%22%5E5)%20OR%20(%22education.postsecondary.nursing%22%5E10%20OR%20%22education.postsecondary%22%5E5%20OR%20%22education%22))");
		listings.put("16.7",
				"/jobs/mlt?df=jobdescription&fl=id,jobtitle&rows=5&q=J2EE&mlt.fl=jobtitle,jobdescription&mlt.interestingTerms=details");
		listings.put(
				"16.8",
				"/jobs/mlt?df=jobdescription&q=id:fc57931d42a7ccce3552c04f3db40af8dabc99dc&mlt.fl=jobtitle,jobdescription&mlt.interestingTerms=details&mlt.boost=true");
		listings.put(
				"16.9",
		...
		listings.put(
				"16.11",
				"/jobs/clustering?q=content:(solr%20OR%20lucene)&fl=id,jobtitle&rows=100&carrot.title=jobtitle&carrot.snippet=jobtitle&LingoClusteringAlgorithm.desiredClusterCountBase=25");
		listings.put(
				"16.12",
				"/jobs/select?df=jobdescription&fl=id,jobtitle&q=(solr%20OR%20lucene)%20OR%20%22Java%20Engineer%22%20OR%20%22Software%20Developer%22");
	}
	...
	public Option[] getOptions() {
		return new Option[] {
				OptionBuilder.withArgName("#").hasArg().isRequired(false)
						.withDescription("Required: Listing #, such as 4.4").withLongOpt("num").create("n"),
				...
				OptionBuilder.withArgName("json|xml|csv").hasArg().isRequired(false)
						.withDescription("Override response writer type parameter \"wt\"").withLongOpt("format")
						.create("f") };
	}
	...
	public void runExample(ExampleDriver driver) throws Exception {
		...
		for (String listing : listingValues) {
			...
			if (listing == null) {
				System.err.println("Sorry, Listing " + listingKey + " is not available for execution.");
				System.exit(1);
			}
