...
public class DistanceFacetDocGenerator implements ExampleDriver.Example {
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFile));) {
			...
			for (WeightedLocation location : locations) {
				for (Integer i = 0; i < location.numDocs; i++) {
					...
					doc.append("    <field name=\"location\">" + changeLastDigit(location.latitude) + ","
							+ changeLastDigit(location.longitude) + "</field>\n");
	...
	private static Double changeLastDigit(Double numberToChange) {
		String newDouble = numberToChange.toString().substring(0, numberToChange.toString().length() - 1)
				+ random.nextInt(9);
	...
	public Option[] getOptions() {
		return new Option[] { OptionBuilder.withArgName("file").hasArg().isRequired(false)
