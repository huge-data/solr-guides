...
public class IndexLog extends ExampleDriver.SolrJClientExample {
	...
	public static Logger log = LoggerFactory.getLogger(IndexLog.class);
	...
	private final StringBuilder multilineBuf = new StringBuilder();
		...
		try {
			...
			if (rootCause instanceof IOException) {
				log.error(rootCause.getClass().getSimpleName()
						+ " when trying to send a document to SolrCloud, will re-try after waiting " + retryInSecs
						+ " seconds; " + rootCause);
				try {
	...
	protected SolrInputDocument parseSolrLogLine(String line, int lineNum) {
		...
		String docId = String.format("%s/%d/%s/%d/%s", "localhost", lineNum, level, timestampDt.getTime(),
