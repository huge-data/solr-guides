...
public class ClusterStateAwarePingRequestHandler extends PingRequestHandler {
	...
	protected void verifyThisReplicaIsActive(SolrCore solrCore) throws SolrException {
		...
		if ("active".equals(replicaState)) {
			log.info(String.format("%s at %s for %s in the %s collection is active.", role, nodeName, shardName,
					collectionName));
		} else {
			// fail the ping by raising an exception
			String errMsg = String.format("%s at %s for %s in the %s collection is not active! State is: %s", role,
					nodeName, shardName, collectionName, replicaState);
			if (exc != null) {
