...
public class MultiTextFieldLanguageIdentifierUpdateProcessor extends LangDetectLanguageIdentifierUpdateProcessor {

	protected final static Logger log = LoggerFactory.getLogger(MultiTextFieldLanguageIdentifierUpdateProcessor.class);
	...
	public MultiTextFieldLanguageIdentifierUpdateProcessor(SolrQueryRequest req, SolrQueryResponse rsp,
	...
	private void initParams(SolrParams params) {
		...
		if (prependGranularity != null && prependGranularity.trim().length() > 0) {
			if (prependGranularity.trim().equals("document")) {
				...
				log.error("Unsupported format for" + PREPEND_GRANULARITY + ":" + prependGranularity + ". Using "
		...
		for (String nextFieldName : this.prependFields) {
			if (indexSchema.getFieldOrNull(nextFieldName) != null) {
				if (indexSchema.getField(nextFieldName).getType() instanceof MultiTextField) {
					...
					log.error("Invalid field " + PREPEND_FIELDS + ":" + nextFieldName + ". Field is not a "
							+ MultiTextField.class + ".");
				}
			} else {
