...
public class MultiTextField extends TextField {
	...
	protected void init(IndexSchema schema, Map<String, String> args) {
		...
		MultiTextFieldSettings indexSettings = new MultiTextFieldSettings(AnalyzerModes.index, keyFromTextDelimiter,
				multiKeyDelimiter, defaultFieldTypeName, fieldMappings, ignoreMissingMappings, removeDuplicates,
				hidePrependedLangs);

		MultiTextFieldSettings querySettings = new MultiTextFieldSettings(AnalyzerModes.query, keyFromTextDelimiter,
				multiKeyDelimiter, defaultFieldTypeName, fieldMappings, ignoreMissingMappings, removeDuplicates,
				hidePrependedLangs);

		MultiTextFieldSettings multiTermSettings = new MultiTextFieldSettings(AnalyzerModes.multiTerm,
				keyFromTextDelimiter, multiKeyDelimiter, defaultFieldTypeName, fieldMappings, ignoreMissingMappings,
	...
	public IndexableField separateIndexableFromStorableValue(SchemaField field, String indexableValue,
			String storableValue, float boost) throws Exception {
		if (indexableValue == null || indexableValue.trim().length() == 0 || storableValue == null
				|| storableValue.trim().length() == 0) {
			return null;
		}
		...
		if (field.indexed()) {
			...
			TokenStream indexableTokenStream = ((MultiTextFieldAnalyzer) this.getAnalyzer()).createComponents(
					field.getName(), new StringReader(indexableValue)).getTokenStream();
			if (f != null) {
	...
	private String indexableToStorable(String indexableValue) {
		...
		if (indexableValue.startsWith("[") && indexableValue.contains("|]")) {
			storableValue = indexableValue.substring(indexableValue.indexOf("|]") + "|]".length(),
					indexableValue.length() - 1);
		}
